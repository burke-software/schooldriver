...
@user_passes_test(lambda u: u.has_perm('sis.change_studentattendance'))
def attendance_bulk_change(request):
    if request.method == 'POST':
        form = AttendanceBulkChangeForm(request.POST)
        if form.is_valid():
            ...
            for attendance in StudentAttendance.objects.filter(id__in=ids):
                if data['date']:
                ...
                if data['private_notes']:
                    attendance.private_notes = data['private_notes']
                attendance.save()
            ...
            messages.success(request, 'Attendance bulk change successful')
            return HttpResponseRedirect(reverse('admin:sis_studentattendance_changelist'))
    else:
        form = AttendanceBulkChangeForm()
    ...
    return render_to_response('sis/generic_form.html', {'request': request, 'form': form, \
        'title': 'Bulk Attendance Change', 'msg': msg})

@user_passes_test(lambda u: u.has_perm('sis.change_student'))
def student_bulk_change(request, queryset=None):
    if request.method == 'POST':
        form = StudentBulkChangeForm(request.POST)
        if form.is_valid():
            if queryset:
                students = queryset
            else:
            ...
            for student in students:
                ...
                if data['individual_education_program'] != None:
                    student.individual_education_program = data['individual_education_program']
                if data['cohort']:
                    student_cohort = StudentCohort.objects.get_or_create(student_id=student.id, cohort_id=data['cohort'].id)[0]
                    student_cohort.primary = data['cohort_primary']
                    student_cohort.save()
                if data['award']:
                    AwardStudent.objects.get_or_create(student_id=student.id, marking_period_id=data['award_marking_period'])
                student.save()
                LogEntry.objects.log_action(
                    user_id         = request.user.pk, 
            ...
            messages.success(request, 'Student bulk change successful')
            return HttpResponseRedirect(reverse('admin:sis_student_changelist'))
    else:
        form = StudentBulkChangeForm()
    msg = 'Leaving field blank will not make changes to that field. You cannot bulk delete awards or cohorts you may only add them. '
    return render_to_response('sis/generic_form.html', {'request': request, 'form': form, \
        'title': 'Bulk Student Change', 'msg': msg})
