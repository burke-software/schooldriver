...
def teacher_grade_upload(request, id):
    ...
    if request.method == 'POST' and 'upload' in request.POST:
        ...
        #import_form.fields['marking_period'].queryset = import_form.fields['marking_period'].queryset.filter(course_section=course)
    ...
    for mp in marking_periods:
        for student in students:
            grade = Grade.objects.get_or_create(student=student, course_section=course, marking_period=mp)
    
    for student in students:
        student.grades = student.grade_set.filter(course_section=course, override_final=False)
        try:
            student.final = student.grade_set.get(course_section=course, override_final=True).get_grade()
