...
class GradeCalculationTests(SisTestMixin, TestCase):
    ...
    def test_basic_grades(self):
        ...
        test_data = [
            ...
            [student, course, mp3, 90.5, 85],
        ]
        for data in test_data:
            enrollment = CourseEnrollment.objects.get(
                user=data[0], course_section=data[1])
            grade = Grade.objects.get_or_create(
                enrollment=enrollment, marking_period=data[2])[0]
            grade.set_grade(data[3])
            grade.save()
            average = Grade.get_course_grade(enrollment)
            self.assertAlmostEquals(average, data[4])
