...
class GradeTestTCSampleData(TestCase):
    ...
    def test_student_2_year_grades(self):
        student = self.data.tc_student2
        year_grade_1 = self.calc.get_year_average(student, self.data.year1)
        year_grade_2 = self.calc.get_year_average(student, self.data.year2)
        self.assertEqual(year_grade_1, 3.26)
        self.assertEqual(year_grade_2, 3.43)
