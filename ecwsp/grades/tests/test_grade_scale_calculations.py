...
class GradeScaleTests(SisTestMixin, TestCase):
    def setUp(self):
        ...
        GradeScaleRule.objects.create(
            min_grade=50, max_grade=Decimal('59.99'), letter_grade='F',
            numeric_scale=1, grade_scale=scale)
        GradeScaleRule.objects.create(
            min_grade=60, max_grade=69.99, letter_grade='D', numeric_scale=1.5,
            grade_scale=scale)
        GradeScaleRule.objects.create(
            min_grade=70, max_grade=79.99, letter_grade='C', numeric_scale=2,
            grade_scale=scale)
        GradeScaleRule.objects.create(
            min_grade=80, max_grade=89.99, letter_grade='B', numeric_scale=3,
            grade_scale=scale)
        GradeScaleRule.objects.create(
            min_grade=90, max_grade=90, letter_grade='A', numeric_scale=4,
            grade_scale=scale)
    ...
    def test_grade_scale(self):
        ...
        grade = Grade.objects.get(
            enrollment__user=self.data.student2,
            enrollment__course_section=self.data.course_section,
