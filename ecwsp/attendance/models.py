...
def post_save_attendance_handler(sender, instance, **kwargs):
    ...
    try:
        # Create discipline record if student is tardy and various config settings are met
        if (Configuration.get_or_default("attendance_create_discipline", "False").value == "True" and
            'ecwsp.discipline' in settings.INSTALLED_APPS and
            instance.date == datetime.date.today() and
            instance.status.tardy and
            not instance.status.excused
            ):
            from ecwsp.discipline.models import StudentDiscipline, DisciplineAction, DisciplineActionInstance, Infraction
            tardies_before_disc = int(Configuration.get_or_default("attendance_tardies_before_disc", "1").value)
            year = SchoolYear.objects.get(active_year=True)
            if (instance.student.student_attn.filter(
                    status__tardy=True,
                    status__excused=False,
                    date__range=(year.start_date,year.end_date)
                ).count() >= tardies_before_disc and
                not StudentDiscipline.objects.filter(
                    students=instance.student,date=datetime.date.today()
                ).count()
                ):
                ...
                disc.save()
                disc.students.add(instance.student)
